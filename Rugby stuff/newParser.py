    ...
    with open(os.path.join("datas", name)) as file:
        ...
        try:
            if len(allTeams[j["RRML"]["TeamDetail"]["Team"][0]["@attributes"]["team_name"]].previousStats.items()) == 0:
                ...
                allTeams[j["RRML"]["TeamDetail"]["Team"][0]["@attributes"]["team_name"]].currentSeason = str(j["RRML"]["@attributes"]["season_id"])
            ...
            if allTeams[j["RRML"]["TeamDetail"]["Team"][1]["@attributes"]["team_name"]].previousStats == {}:
                ...
                allTeams[j["RRML"]["TeamDetail"]["Team"][1]["@attributes"]["team_name"]].currentSeason = \
                j["RRML"]["@attributes"]["season_id"]
        except:
            print(sys.exc_info())
        ...
        if(str(allTeams[home].currentSeason) == j["RRML"]["@attributes"]["season_id"]):
            line += str(allTeams[home].currentGame) + ","
            allTeams[home].currentGame += 1
        else:
            line += "1,"
            allTeams[home].currentGame = 2
            allTeams[home].currentSeason = str(j["RRML"]["@attributes"]["season_id"])

        if(str(allTeams[away].currentSeason) == j["RRML"]["@attributes"]["season_id"]):
            line += str(allTeams[away].currentGame) + ","
            allTeams[away].currentGame += 1
        else:
            line += "1,"
            allTeams[away].currentGame = 2
            allTeams[away].currentSeason = str(j["RRML"]["@attributes"]["season_id"])



        line += j["RRML"]["TeamDetail"]["Team"][0]["@attributes"]["home_or_away"] + ","                         #known pre game
        ...
        outfile.write(line + "\n")                                                                  #HERE THE LINE IS WRITTEN, NO VALUES ARE UPDATED AFTERWARDS
